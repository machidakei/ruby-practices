...
if @options["l"]
  array = Dir.glob('*').sort
  filetypes = array.map { |file| File::Stat.new(file).ftype }
  ...
  file_access_authorization = permitted_attributes.map do |permitted_attribute| 
    file_access_authorization_list[permitted_attribute[0]] + file_access_authorization_list[permitted_attribute[1]] + file_access_authorization_list[permitted_attribute[2]]
  end
  filemode_and_file_access_authorization = filemodes.map.with_index do |filemode, i|
    filemode + file_access_authorization[i]
  end
  number_of_hard_link = array.map { |file| File::Stat.new(file).nlink.to_s }
  user_name = array.map { |file| Etc.getpwuid(File::Stat.new(file).uid).name }
  group_name = array.map { |file| Etc.getgrgid(File::Stat.new(file).gid ).name }
